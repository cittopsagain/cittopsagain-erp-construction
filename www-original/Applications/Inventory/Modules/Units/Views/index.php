<script type="text/javascript">
    Ext.define('App.view.units.Grid', {
        extend: 'Ext.grid.Panel',
        alias: 'widget.units-grid',
        ...
        store: {
            ...
            proxy: {
                type: 'ajax',
                url: '<?php echo rtrim(BASE_URL, '/'); ?>/Inventory/Units/Main/data',
                reader: {
                    type: 'json',
                    rootProperty: 'data',
                    totalProperty: 'total'
            ...
            },
            {
                ...
                handler: function () {
                    ...
                    if (selection.length > 0) {
                        ...
                        Ext.Msg.confirm('Delete', 'Are you sure you want to delete this unit?', function (choice) {
                            if (choice === 'yes') {
                                editing.cancelEdit();
                                if (record.phantom) {
                                    store.remove(record);
                                    ...
                                    Ext.Ajax.request({
                                        url: '<?php echo rtrim(BASE_URL, '/'); ?>/Inventory/Units/Main/delete',
                                        method: 'POST',
        ...
        listeners: {
            selectionchange: function (model, records) {
                this.down('#removeUnit').setDisabled(!records.length);
            }
        }
    });
</script>
